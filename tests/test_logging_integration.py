...
def test_weather_fallback_logs_warning(monkeypatch, caplog):
    ...
    context = weather.get_weather_context(40.4168, -3.7038, "Europe/Madrid", date(2026, 7, 15))
...
def test_analysis_logs_start_and_end(base_request, caplog):
    ...
    messages = [record.message for record in caplog.records if record.name == "app.services.analysis"]
...
def test_recommendation_logs_summary(base_request, caplog):
    ...
    messages = [record.message for record in caplog.records if record.name == "app.services.recommendations"]
...
def test_apply_pending_session_patch_logs_info(monkeypatch, caplog):
    ...
    fake_streamlit = SimpleNamespace(session_state={forms.PENDING_SESSION_PATCH_KEY: {"desk_x_m": 1.85}})
    ...
    assert any("event=session_patch_applied" in record.message for record in caplog.records)
