...
def _movement_message(delta_x: float, delta_y: float) -> str | None:
    ...
    if abs(delta_x) >= 0.05:
        ...
        moves.append(f"desplaza el escritorio {abs(delta_x) * 100:.0f} cm hacia el {horizontal}")
    if abs(delta_y) >= 0.05:
...
def _rotation_message(delta_deg: float) -> str | None:
    ...
    return f"gira la mesa {abs(delta_deg):.0f} deg {direction}"
...
def recommend_variant(
            ...
            for monitor_rotation in _candidate_monitor_rotations():
                candidate_request = replace(
                    ...
                    desk=replace(
                        ...
                        orientation_deg=(request.desk.orientation_deg + desk_rotation) % 360,
                    ),
                    monitor=replace(
                        request.monitor,
                        x_m=round(request.monitor.x_m + delta_x, 2),
                        y_m=round(request.monitor.y_m + delta_y, 2),
                        orientation_deg=(request.monitor.orientation_deg + desk_rotation + monitor_rotation)
                        % 360,
                    ),
                )
                try:
                ...
                if candidate_result.comfort_score > best_result.comfort_score + 0.2:
                    if env_flag("SUNSETUP_DEBUG_CANDIDATES") and logger.isEnabledFor(logging.DEBUG):
                        log_event(
                            logger,
                            logging.DEBUG,
    ...
    desk_delta = ((best_result.request.desk.orientation_deg - request.desk.orientation_deg + 540) % 360) - 180
    rotation_message = _rotation_message(desk_delta)
    monitor_delta = (
        ((best_result.request.monitor.orientation_deg - request.monitor.orientation_deg + 540) % 360) - 180
    )
    monitor_message = (
        f"ajusta el monitor {abs(monitor_delta):.0f} deg {'a la derecha' if monitor_delta > 0 else 'a la izquierda'}"
        if abs(monitor_delta) >= 1
        else None
    )
    recommendation_messages = [
        message for message in (movement_message, rotation_message, monitor_message) if message
    ]
    if not recommendation_messages:
        recommendation_messages = ["La configuracion actual ya esta bastante equilibrada para este modelo"]

    if best_result.request.include_seasonal_summary is False and request.include_seasonal_summary:
    ...
    best_result.alerts = [*best_result.alerts, f"Mejora estimada del confort: +{delta_score:.1f} puntos."]
    log_event(
        logger,
        logging.INFO,
